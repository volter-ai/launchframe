# Launchframe Launch Context

This is the clean dogfood context for Launchframe's own public beta.

## Verified Surfaces

| Surface | Target |
|---|---|
| Repo | `https://github.com/volter-ai/launchframe` |
| Package | `launchframe-cli` |
| Install | `npx launchframe-cli` |
| Site | `https://launchframe.site/` |
| Startup Pilot | `https://launchframe.site/startup-pilot.html` |

## Capture

Launchframe uses a low-friction Tally capture form:

- Email required.
- "How did you hear about Launchframe?" optional.
- No payment collected in the form.
- Self-email notification to the operator verified.

## Claim

Launchframe turns scattered launch work into a structured, auditable workspace with templates, validation, evidence, and a launch-day runbook.
